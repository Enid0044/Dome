var tools = (function(){
	var toolsObj = {
		$:function(selector,context){   //获取元素
			/*
			...
			if(selector.indexOf(" ") !== -1){  //.class a
				return context.querySelectorAll(selector);
			}else if( selector.charAt(0) === "#" ){
				return document.getElementById(selector.slice(1))   //id
			}else if( selector.charAt(0) === "." ){
				return context.getElementsByClassName(selector.slice(1));  //class
			}else{
				return context.getElementsByTagName(selector);  //标签
			}
		},
		addEvent:function(ele,eventName,eventFn){
			ele.addEventListener(eventName,eventFn,false); //事件绑定函数
		},
		removeEvent:function(ele,eventName,eventFn){
			ele.removeEventListener(eventName,eventFn,false);   //取消时间绑定函数
		},
		addClass:function (element,clsNames){    //添加class，  元素没有这个class才添加
		...
		removeClass:function (element,clsNames){  //删除class
		...
		hasClass:function(ele,classNames){   //判断元素是否有这个class，  有ture,没有false
		...
		toggleClass:function (ele,classNames){   //开关
			if( tools.hasClass(ele,classNames) ){   //如果有就删除false
				tools.removeClass(ele,classNames);
				return false;
			}else{
				tools.addClass(ele,classNames);//如果没有就添加，返回true
				return true;
			}
		},
		parents:function(obj,selector){   //找指定的父级
			/*
		...
		getEleRect:function(obj){      //获取内容高度
